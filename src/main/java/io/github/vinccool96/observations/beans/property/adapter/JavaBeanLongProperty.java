...
/**
 * A {@code JavaBeanLongProperty} provides an adapter between a regular Java Bean property of type {@code long} or
 * {@code Long} and a JavaFX {@code LongProperty}. It cannot be created directly, but a {@link
 ...
 * JavaBeanLongProperty} will be aware of changes in the Java Bean. Otherwise it can be notified about changes by
 * calling {@link #fireValueChangedEvent()}. If the Java Bean property is also constrained (i.e. it supports
 * VetoableChangeListeners), this {@code JavaBeanLongProperty} will reject changes, if it is bound to an {@link
