...
/**
 * A {@code ReadOnlyJavaBeanBooleanPropertyBuilder} can be used to create {@link ReadOnlyJavaBeanBooleanProperty
 * ReadOnlyJavaBeanBooleanProperties}. To create a {@code ReadOnlyJavaBeanBooleanProperty} one first has to call {@link
 * #create()} to generate a builder, set the required properties, and then one can call {@link #build()} to generate the
 * property.
 * <p>
 * Not all properties of a builder have to specified, there are several combinations possible. As a minimum the {@link
 * #name(String)} of the property and the {@link #bean(Object)} have to be specified. If the name of the getter follows
 * the conventions, this is sufficient. Otherwise it is possible to specify an alternative name for the getter ({@link
 * #getter(String)}) or the getter {@code Methods} directly ({@link #getter(Method)}).
 * <p>
 ...
 * ReadOnlyJavaBeanBooleanPropertyBuilder}. by switching the Java Bean instance (with {@link #bean(Object)} and calling
 * {@link #build()}.
 *
...
public final class ReadOnlyJavaBeanBooleanPropertyBuilder {
     ...
     *         if the Java Bean property is not of type {@code boolean} or {@code Boolean}
     */
    ...
    /**
     * Set the Java Bean class in which the getter should be searched. This can be useful, if the builder should
     * generate adapters for several Java Beans of different types.
     *
     * @param beanClass
     *         the Java Bean class
     *
