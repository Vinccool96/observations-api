...
/**
 * A {@code JavaBeanIntegerProperty} provides an adapter between a regular Java Bean property of type {@code int} or
 * {@code Integer} and a JavaFX {@code IntegerProperty}. It cannot be created directly, but a {@link
 * JavaBeanIntegerPropertyBuilder} has to be used.
 * <p>
 ...
 * JavaBeanIntegerProperty} will be aware of changes in the Java Bean. Otherwise it can be notified about changes by
 * calling {@link #fireValueChangedEvent()}. If the Java Bean property is also constrained (i.e. it supports
 * VetoableChangeListeners), this {@code JavaBeanIntegerProperty} will reject changes, if it is bound to an {@link
 * ObservableValue ObservableValue&lt;Integer&gt;}.
 *
 * @see IntegerProperty
