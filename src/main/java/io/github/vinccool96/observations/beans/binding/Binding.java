...
/**
 * A {@code Binding} calculates a value that depends on one or more sources. The sources are usually called the
 * dependency of a binding. A binding observes its dependencies for changes and updates its value automatically.
 * <p>
 * While a dependency of a binding can be anything, it is almost always an implementation of {@link ObservableValue}.
 * {@code Binding} implements {@code ObservableValue} allowing to use it in another binding. With that one can assemble
 * very complex bindings from simple bindings.
 * <p>
 ...
 * <p>
 * It is recommended to use one of the base classes defined in this package (e.g. {@link DoubleBinding}) to define a
 * custom binding, because these classes already provide most of the needed functionality. See {@link DoubleBinding} for
 * an example.
 *
 * @see DoubleBinding
*/
public interface Binding<T> extends ObservableValue<T> {
     ...
     *
     * @return {@code true} if the {@code Binding} is valid, {@code false} otherwise
     */
    boolean isValid();

    /**
     * Mark a binding as invalid. This forces the recalculation of the value of the {@code Binding} next time it is
     * request.
     */
    void invalidate();
    ...
    /**
     * Signals to the {@code Binding} that it will not be used anymore and any references can be removed. A call of this
     * method usually results in the binding stopping to observe its dependencies by unregistering its listener(s). The
     * implementation is optional.
     * <p>
     * All bindings in our implementation use instances of {@link io.github.vinccool96.observations.beans.WeakInvalidationListener},
     * which means usually a binding does not need to be disposed. But if you plan to use your application in
     * environments that do not support {@code WeakReferences} you have to dispose unused {@code Bindings} to avoid
     * memory leaks.
     */
    void dispose();
