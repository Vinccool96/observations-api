...
/**
 * A {@code JavaBeanBooleanProperty} provides an adapter between a regular Java Bean property of type {@code boolean} or
 * {@code Boolean} and a JavaFX {@code BooleanProperty}. It cannot be created directly, but a {@link
 ...
 * JavaBeanBooleanProperty} will be aware of changes in the Java Bean. Otherwise it can be notified about changes by
 * calling {@link #fireValueChangedEvent()}. If the Java Bean property is also constrained (i.e. it supports
 * VetoableChangeListeners), this {@code JavaBeanBooleanProperty} will reject changes, if it is bound to an {@link
