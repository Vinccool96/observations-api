...
import io.github.vinccool96.observations.sun.property.JavaBeanAccessHelper;
...
/**
 * A binding used to get a member, such as <code>a.b.c</code>. The value of the binding will be "c", or null if c could
 * not be reached (due to "b" not having a "c" property, or "b" being null). "a" must be passed to the constructor of
 * the SelectBinding and may be any dependency. All subsequent links are simply PropertyReferences.
 * <p>
 * With a SelectBinding, "a" must always exist. Usually "a" will refer to "this", or some concrete object. "b"* will be
 * some intermediate step in the select binding.
 */
public class SelectBinding {
    ...
    private static class SelectBindingHelper implements InvalidationListener {
        ...
        public ObservableList<ObservableValue<?>> getDependencies() {
            if (dependencies == null) {
                dependencies = ObservableCollections.observableArrayList();
                updateDependencies();
            }

            return ObservableCollections.unmodifiableObservableList(dependencies);
        }
