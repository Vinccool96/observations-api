...
/**
 * Abstract class that serves as a base class for {@link ObservableList} implementations. The base class provides two
...
public abstract class ObservableListBase<E> extends AbstractList<E> implements ObservableList<E> {
    ...
    /**
     * Adds a new set operation to the change. Equivalent to {@code nextRemove(idx); nextAdd(idx, idx + 1); }.
     * <p><strong>Note</strong>: needs to be called inside {@code beginChange()} / {@code endChange()} block.
     * <p><strong>Note</strong>: needs to reflect the <em>current</em> state of the list.
     *
     * @param idx
     ...
     * @param old
     *         the old value at the {@code idx} position.
     */
    ...
    /**
     * Adds a new replace operation to the change. Equivalent to {@code nextRemove(from, removed); nextAdd(from, to); }
     * <p><strong>Note</strong>: needs to be called inside {@code beginChange()} / {@code endChange()} block.
     * <p><strong>Note</strong>: needs to reflect the <em>current</em> state of the list.
     *
     * @param from
    ...
    /**
     * Adds a new permutation operation to the change. The permutation on index {@code "i"} contains the index, where
     * the item from the index {@code "i"} was moved.
     ...
     * @param perm
     *         the permutation in that range. Even if {@code from != 0}, the array should contain the indexes of the
     *         list. Therefore, such permutation would not contain indexes of range {@code (0, from)}
     */
     ...
     * <p>
     * Must be called before any of the {@code next*} methods is called. For every {@code beginChange()}, there must be
     * a corresponding {@link #endChange() } call.
     ...
     * <p>
     * If the block is the outer-most block for the {@code ObservableList}, the {@code Change} is constructed and all
