...
/**
 * A {@code WeakMapChangeListener} can be used, if an {@link ObservableMap} should only maintain a weak reference to the
 * listener. This helps to avoid memory leaks, that can occur if observers are not unregistered from observed objects
 * after use.
 * <p>
 * {@code WeakMapChangeListener} are created by passing in the original {@link MapChangeListener}. The {@code
