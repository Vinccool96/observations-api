...
public class When_Integer_Test extends WhenTestBase<Number, Property<Number>> {
    ...
    public When_Integer_Test() {
        super(
                ...
                new SimpleIntegerProperty(), new SimpleDoubleProperty(), new SimpleFloatProperty(),
                new SimpleLongProperty(), new SimpleIntegerProperty()
        );
    }
    ...
    public Binding<Number>[] generatePropertyPropertyList(Property<Number> p0, Property<Number>[] props) {
        ...
        for (int i = 0; i < n; i++) {
            result[i] =
                    Bindings.when(cond).then((ObservableNumberValue) p0).otherwise((ObservableNumberValue) props[i]);
        }
        return result;
    }
