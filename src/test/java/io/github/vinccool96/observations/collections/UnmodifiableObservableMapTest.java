...
public class UnmodifiableObservableMapTest {
    ...
    public void setUp() throws Exception {
        ...
        unmodifiableMap = ObservableCollections.unmodifiableObservableMap(observableMap);
    ...
    /**
     * Modifies the map in the fixture to use the strings passed in instead of the default strings, and re-creates the
     * observable map and the observer. If no strings are passed in, the result is an empty map.
     *
     * @param strings
    ...
    public void testEntrySet() {
        ...
        try {
            unmodifiableMap.entrySet()
                    .removeAll(Arrays.asList(entry("one", "1"), entry("two", "2"), entry("three", "3")));
            fail("Expected UnsupportedOperationException");
        } catch (UnsupportedOperationException e) {
        }
        try {
            unmodifiableMap.entrySet()
                    .retainAll(Arrays.asList(entry("one", "1"), entry("two", "2"), entry("three", "3")));
            fail("Expected UnsupportedOperationException");
        } catch (UnsupportedOperationException e) {
        }
        try {
