...
public class BindingsCreateBindingTest<T> {
    ...
    public BindingsCreateBindingTest(Property<T> p0, Property<T> p1, Functions<T> f, T value0, T value1,
            T defaultValue) {
        this.p0 = p0;
        this.p1 = p1;
        this.f = f;
    ...
    public void testCreateBoolean_TwoDependencies() {
        ...
        f.check(p0.getValue(), binding.getValue());
        assertTrue(binding.getDependencies().equals(Arrays.asList(p0, p1)) ||
                binding.getDependencies().equals((Arrays.asList(p1, p0))));
        p0.setValue(value1);
        f.check(p0.getValue(), binding.getValue());
        binding.dispose();
    }
