...
public class SelectBindingTest {
    ...
    /**
     * This test performs 10,000 random operations on the chain of a.b.c (setting different values for each step,
     * sometimes doing multiple assignments between get() calls, random invalidate() calls). After each random
     * iteration, we check to see if the listeners installed on the a, b, c objects are still correct. The goal is to
     * catch any freak situations where we might have installed two listeners on the same property, or failed to remove
     * a listener for a property under some circumstance.
     * <p>
     * Do note that the only listeners that are installed in this method are done by the select binding.
     */
    @SuppressWarnings("unchecked")
    @Test
